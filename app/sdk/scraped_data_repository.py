...
class ScrapedDataRepository:
    ...
    def download_img(self, source_data: KernelPlancksterSourceData, job_id: int, file_path: str) -> KernelPlancksterSourceData:
        ...
        match self.protocol:
            ...
            case ProtocolEnum.S3:
                ...
                self.logger.info(f"{job_id}: Downloading img from object store")

                self.file_repository.public_download(signed_url, file_path)
                
                self.logger.info(
                f"{job_id}: Downloaded img to {file_path}"
                )       
